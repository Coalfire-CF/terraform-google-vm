...
/*************************************************
 Separate file system hardening
*************************************************/

# resource "google_compute_disk" "separate_filesystem" {
#   count   = var.windows == false ? var.num_instances : 0
#   project = var.project_id

#   name     = "${var.name}${count.index + 1}-sdb"
#   snapshot = "${reverse(split("/", var.source_image))[0]}-sdb"
#   type     = var.disk_type
#   size     = 50
#   zone     = element(var.zones, count.index)
...
# resource "google_compute_attached_disk" "separate_filesystem" {
#   project = var.project_id
#   count   = var.windows == false ? var.num_instances : 0

#   instance    = google_compute_instance.compute_instance[count.index].id
#   disk        = google_compute_disk.separate_filesystem[count.index].id
#   device_name = "sdb"
# }

# resource "google_compute_disk_resource_policy_attachment" "separate_filesystem" {
#   project = var.project_id
#   count   = var.windows == false && var.snapshot_schedule != null ? var.num_instances : 0

#   name = var.snapshot_schedule
#   disk = google_compute_disk.separate_filesystem[count.index].name
#   zone = element(var.zones, count.index)
# }
